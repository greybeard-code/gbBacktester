...
- **gbPullback5Bar (2026-09-15) — first genuinely promising, walk-forward-
  ...
  real size.
- **ninZaRenko r112-28 bar-level parity CHECKED (2026-09-16) and mostly
  CONFIRMED GOOD — the "open bug" noted here the same day was a false
  alarm, retracted.** The false alarm came from a trade-timing-only
  comparison against a Market Replay executions export, which looked like
  a single cascading divergence starting 2026-09-03. A follow-up
  bar-level check against a real `gbBarExporter` chart export (MNQ,
  Brick=112/Reversal=28, 2026-08-23..2026-09-16, ~2,000 bars) instead found
  **99%+ identical bars on 17 of 18 days**, including the exact
  2026-09-02..09-03 window the trade comparison had flagged (zero bar
  mismatches there) — so `build_renko_bars` at this size is not broken.
  One real, isolated anomaly: 2026-09-10's first post-midnight bar shows
  NT8 resetting (body = exactly `trend` ticks, the fresh-reset signature)
  where our side — correctly carrying state from 9/9 — built a
  continuation bar; our own recorded ticks show no gap there, so this
  looks like a live-feed hiccup specific to that one NT8 Market Replay
  session, not a flaw in our gap-based reset logic. **Debugging gotcha
  worth remembering**: `Catalog.load_bars_sequence`'s cache-hit path
  returns a `BarDay` whose own `.end_anchor`/`.end_dir` fields are always
  the dataclass defaults (0.0/0) — the real carry-out is threaded via
  `_load_renko_cached`'s separate scalar return values, never written onto
  that object. Reading `.end_anchor` off a returned `BarDay` for debugging
  is exactly the kind of check that manufactures a fake "state reset" and
  cost real time here; verify via the on-disk parquet cache's
  `carry_anchor`/`carry_dir`/`end_anchor`/`end_dir` metadata instead
  (`pyarrow.parquet.read_table(...).schema.metadata`), or via a from-scratch
  manual thread through `build_renko_bars` bypassing the cache entirely.
  **Real open question now, much narrower than before**: since bars check
  out, the gbPullback5Bar trade-level mismatch must come from fill/exit
  mechanics — the trades-CSV comparison's `stop_offset_ticks=2` was a
  guess, never confirmed against the live run. Nobody has re-run that
  comparison with a swept `stop_offset_ticks` against now-verified-correct
  bars; do that before concluding anything further about r112-28's live
  fidelity.
